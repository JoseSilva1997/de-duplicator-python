...
The catch-all strategy at the end of the pipeline."""
...
def fuzzy_name(
    ...
    """Match candidates against secondary records on name alone.
    ...
    of that pass receive the first-name check. Survivors are processed in original
    secondary order, so the best-match tie-break matches what a plain double loop would
    produce.
    """
